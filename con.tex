...
В конечном итоге мы хотели бы написать \textsc{wait-free} бор, который будет способен быстро обрабатывать запросы, не ломаться в случаях аварийных завершений и разумно работать с оперативной памятью
...
int fetch_add() {
    ...
    return cur_memory;
		...
		\end{lstlisting}
		Данный пример НЕ является \textsc{obstruction-free}, потому что система не будет продвигаться в случае выключения потоков в процессе работы. \textsc{obstruction-free} являются методы, которые, например, сначала используют несколько CAS, и в случае отказа хотя бы одного выполняют операцию снова. Такая система будет продвигаться если в процессе работы выключить некоторые потоки, поэтому оно корректно
...
\begin{enumerate}
	...
	\item Представление необходимых принципов для структур \href{https://oasis.library.unlv.edu/cgi/viewcontent.cgi?article=3425\&context=thesesdissertations}{https://oasis.library.unlv.edu/cgi/viewcontent.cgi?article=3425\&context=thesesdissertations}
