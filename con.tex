...
В конечном итоге мы хотели бы написать \textsc{wait-free} самобалансирующееся дерево, которое будет способно быстро обрабатывать запросы, не ломаться в случаях аварийных завершений и разумно работать с оперативной памятью.
...
Result do_operation(Operation x) {
	...
	while (!announce[i].is_done()) {
		Operation announce' = announce.copy();
		Rx' = Rx.copy();
		newRx' = Rx';
		for (int i : {0..P-1}) {
			if (!announce'[i].is_done()) {
				// next rule is required to achieve wait-freedom
				if (announce'[i].time <= x.time) {
					newRx'->announce'[i](); // do an operation
					announce'[i].done();
				}
			}
		}
...
Можно заметить что такое решение работает за \emph{$O(P^2T)$} сложность на каждом из потоков, хоть и является \textsc{wait-free}. Необходимо придумать что-то похожее, но оптимальнее. Также можно заметить, что невозможно отменить операцию, даже если у неё был сделан анонс - по крайней мере здесь это является достаточно сложной задачей. Попробуем применить подход с нумерацией операций.
...
\begin{lstlisting}
Queue<Operation> announces;
Result do_operation(Operation x) {
	x.time = operation_counter->increment();
	announces.push(x);
	while (!x.is_done()) {
		Operation announce' = announces.front();
		Rx' = Rx.copy();
		newRx' = Rx';
		if (announce'.time <= x.time) { // should be always true
			newRx'->announce'(); // do an operation
			announce'.done();
		}
		CAS(Rx, Rx', newRx');
		announce'->done(); // for !x.is_done() rule

		// if announce.front() is announce' then pop
		// otherwise someone else removed this
		announce.pop(announce');
		free(announce');
	}
	return done;
}
...
Наша асимптотика стала \emph{$O(PT)$}, чего мы и хотели добиться. Заметим, что для начала нам нужна \textsc{wait-free} очередь, а также механизм освобождения памяти, выделенной под анонс, чтобы реализовать данный подход. Написание \textsc{wait-free} аллокации памяти не является тривиальной задачей, попробуем использовать преобразование очереди. 

Нам по факту не важно будет ли поток, который первым добавился в очередь, исполнен первым, нам нужно лишь чтобы все операции рано или поздно были исполнены. Тогда пусть наша очередь будет расположена на массиве, и у нас будет единый счетчик текущей исполняемой операции. Назовем его \emph{gate}, также в нём же будем хранить количество последних исполненных операций, т.е. наш \emph{operation\_counter}.
...
\begin{lstlisting}
struct Gate {
	size_t cur; // pointer - current active operation
	size_t seq; // counter - number of done operations
};
Operation announce[P];
Gate gate = {nullptr, 0};
...
Result do_operation(Operation x) {
	...
	while (!x.is_done()) {
		gate' = gate.load();
		if (gate'.cur == nullptr) {
			Operation announce' = announce[gate'.seq % P];
			if (announce'.is_done()) {
				announce' = x;
			}
			if (announce'.is_done()) break;
			CAS(gate, {gate', {announce', gate'.seq}});
			gate' = gate.load();
		}
		if (gate'.cur != nullptr) {
			Operation announce' = announce[gate'.cur];
			// gate' is like a pointer to head of the queue
			// that's why we really need to check
			if (announce'.time <= gate'.seq) { // gate' isn't old
				Rx' = Rx.copy();
				newRx' = Rx';
				newRx'->announce'(); // do an operation
				CAS(Rx, Rx', newRx');
			}
			CAS(announce[gate'.cur], announce', announce'.done());
			CAS(gate, {gate', {nullptr, gate'.seq + 1}});
		}
	}
	if (gate'.cur == i) {
		CAS(gate, {gate', {nullptr, gate'.seq + 1}});
...
Во всех случаях мы упустили один момент: нам нужно обеспечить возможность доступа к старым значениям структуры данных, даже если их только что обновили. Поскольку потоки могут "заснуть"\  ровно перед моментом помощи, нужно сделать две версии структуры, которые будут чередоваться по использованию. Теоретически их количество может быть неограниченным, но больше чем две нам обеспечивать не нужно, ведь после копирования и до выполнения операции мы можем проверить временную корректность данных.

Также необходимо учесть ситуацию, когда один из потоков уже завершил выполнение операции полностью, переключил \emph{toggle} значение в структуре данных, но ещё не выполнил CAS, который фиксирует завершенность \emph{announce[gate'.cur].done()}. И даже после этого стоило бы добавить логику обработки отмены операций, которая должна быть описана примерно в каждой части проверок, из-за чего псевдокод очень сильно увеличится в размере. Поэтому эту часть я описывать не буду.

\begin{lstlisting}
struct Version {
	bool toggle; // version - bit, which resolved current problem
	size_t  seq; // counter - number of done operations
	...
	...
		if (gate'.cur != nullptr) {
			...
			if (announce'.time <= gate'.seq) { // gate' isn't old
				...
				if (version'.seq > gate'.seq) break;

				before = &Rx'[version'.toggle];
				after  = &Rx'[version'.toggle ^ 1];
				if (version'.seq == gate'.seq) { // because CAS(announce)
					swap(*before, *after);
				}

				after = &(*before)->announce'(); // do an operation
				CAS(Rx, Rx', {*before, *after});

				if (version'.seq < gate'.seq)
					CAS(version, version'.toggle, version'.toggle ^ 1);
			}
			CAS(announce[gate'.cur], announce', announce'.done());
			CAS(gate, {gate', {nullptr, gate'.seq + 1}});
		}
...
Если рассуждать грамотно, то проверка на соответствие версий структур должна быть полная, с учетом предыдущего состояния структуры. Для этого стоит не всей структуре дать версию, а каждому элементу в ней. Также в оригинальной статье про универсальные конструкции также рассмотрен способ встроенной и поэтому удобной аллокации. Если есть желание, то предлагаю её на чтение, но данное усложнение лишь мешает пониманию общих принципов, поэтому я его не привожу.

\vfill
\section{\textsc{wait-free} красно-черное дерево}

Теперь мы изучили достаточно и готовы решить итоговую задачу. Для этого см. оригинальную статью Аравинда Натараджана, эта часть должна быть в точности по ней.
