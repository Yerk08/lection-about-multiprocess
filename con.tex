...
Result do_operation(Operation x) {
	...
	announces.push(x);
	while (!x.is_done()) {
		announce' = announces.front();
		Rx' = Rx.copy();
		newRx' = Rx';
		if (announce'.time <= x.time) { // should be always true
			do operation announce' on newRx';
			announce'.done();
		}
		CAS(Rx, Rx', newRx');
...
Наша асимптотика стала \emph{$O(PT)$}, чего мы и хотели добиться. Заметим, что для начала нам нужна \textsc{wait-free} очередь, а также механизм освобождения памяти, выделенной под анонс, чтобы реализовать данный подход. Ни то ни другое не является простой задачей.
...
struct Gate {
	...
	size_t seq; // counter - number of operations
};
Operation announce[P];
Gate gate = {nullptr, 0};
...
Result do_operation(Operation x) {
	...
	while (!x.is_done()) {
		gate' = gate.load();
		if (gate'.cur == nullptr) {
			CAS(gate, {announce[gate'.seq \% P], gate'.seq});
			gate' = gate.load();
		}
		if (gate'.cur != nullptr) {
			announce' = gate'.cur;
			if (announce'.time <= gate'.seq) {
				Rx' = Rx.copy();
				newRx' = Rx';
				do operation announce' on newRx';
				announce'.done();
